...
`monitor.check` in order to be picked up by the _health check runner_. This gives
...
```yml
_monitor:
    resource: "@MonitorBundle/Resources/config/routing.xml"
...
services:
    monitor.check.php_extensions:
        ...
        arguments:
            - [ xhprof, apc, memcache ]
        tags:
            - { name: monitor.check, alias: php_extensions }
```

The important bit there is to remember to tag your services with the `monitor.check` tag.
...
`MonitorBundle\\Composer\\ScriptHandler::checkHealth` ScriptHandler to the
...
"scripts": {
    "post-install-cmd": [
        ...
        "MonitorBundle\\Composer\\ScriptHandler::checkHealth"
    ],
    "post-update-cmd": [
        "Sensio\\Bundle\\DistributionBundle\\Composer\\ScriptHandler::buildBootstrap",
        "Sensio\\Bundle\\DistributionBundle\\Composer\\ScriptHandler::clearCache",
...
First, define an additional reporter service and tag it with `monitor.additional_reporter`:

```yml
my_reporter:
    class: My\Reporter
    tags:
        - { name: monitor.additional_reporter, alias: my_reporter }
```
...
services:
    monitor.check.php_extensions:
        ...
        arguments:
            - [ xhprof, apc, memcache ]
        tags:
            - { name: monitor.check, alias: php_extensions, group: cron }
            - { name: monitor.check, alias: php_extensions, group: app_server }
```
    ...
    checks:
        ...
        groups:
            ...
            name:
                ...
                # Validate that a named extension or a collection of extensions is not available
                php_extensions_not:       [] # Example: xdebug: true
